export const DARK_MODE_TOGGLE_HTML = `
...
export function changeDarkModeListener() {
  // listener for when theme is changed manually
  const checkbox = document.getElementById('darkmode-toggle');
  checkbox.addEventListener('change', () => {
    checkbox.checked ? turnToDarkMode() : turnToLightMode();
  });

  // listener for when theme on system changes
...
export const turnToDarkMode = () => {
  elementList.forEach(cssSelector => {
    try {
      const elements = document.querySelectorAll(cssSelector);
      Array.from(elements).forEach(element => element.classList.add('dark'));
    } catch (err) {}
  });
}

export const turnToLightMode = () => {
  elementList.forEach(cssSelector => {
    try {
      const elements = document.querySelectorAll(cssSelector);
      Array.from(elements).forEach(element => element.classList.remove('dark'));
    } catch (err) {}
  });
}
